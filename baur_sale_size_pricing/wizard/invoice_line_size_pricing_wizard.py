...
class InvoiceLineSizePricingWizard(models.TransientModel):
    ...
    def _compute_price_final(self):
        for wizard in self:
            discount = max(min(wizard.discount or 0.0, 100.0), 0.0)
            wizard.price_final = wizard.price_unit * (1.0 - (discount / 100.0))
    ...
    def _prepare_new_line_vals(self):
        ...
        line_vals = {
            ...
            'discount': max(min(self.discount or 0.0, 100.0), 0.0),
